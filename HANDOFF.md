...
- `기능상세정책.dc.html` — 기능상세정책 (S1~S29 화면별 요소 + D1~D4 개발 명세)
...
2. **Figma 추출본** (`scr/<name>/Components.bundle.js`, `AF*`): 자동 생성 코드. 직접 수정 비권장 — 원본 Figma 재추출 권장. (통합홈=AFHome, 선박리스트=AFReserveList, 새소식기본형=AFNewsA, 스플래쉬=AFSplash)

## 화면 목록 (커스텀)
- 인증: 로그인 · 카카오/네이버 동의 · 휴대폰번호/인증번호 · 약관(미동의/전체동의/상세) · 관심온보딩 · 권한동의
- 메인/탐색: 새소식(피드형) · 통합검색(결과/검색전) · 탐색지도(필터4·피싱포인트시트·길안내→주차장→앱선택[네이버/카카오/T맵]·조황상세·예약업체·CCTV모달)
- 예약: 선박상세 → 예약(인원·날짜) → 결제 → 예약완료
